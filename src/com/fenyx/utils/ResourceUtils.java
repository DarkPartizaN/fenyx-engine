...
public final class ResourceUtils {
    ...
    public static String DIR_ROOT = new File(".").getAbsoluteFile().getParentFile().getAbsolutePath().concat("/");
    ...
    public static String dir_current;
    ...
    public static String[] listFiles(String path) {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (files[i].isDirectory()) names[i] = names[i].concat("/");
        }
    ...
    public static AWTImage loadAWTImage(String path) {
        if (!cached_images.containsKey(path)) {
            try {
                ...
                BufferedImage source_image = ImageIO.read(img);
    ...
    public static void writePNG(String path, ByteBuffer data, int width, int height) {
        byte[] tmpData = new byte[data.remaining()];

        data.get(tmpData);
        DataBuffer buffer = new DataBufferByte(tmpData, tmpData.length);

        WritableRaster raster = Raster.createInterleavedRaster(buffer, width, height, 4 * width, 4, new int[]{0, 1, 2, 3}, null);
        ColorModel cm = new ComponentColorModel(ColorSpace.getInstance(1000), new int[]{8, 8, 8, 8}, true, false, Transparency.TRANSLUCENT, DataBufferByte.TYPE_BYTE);
